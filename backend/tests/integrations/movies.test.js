...
describe('/api/movies', ()=>{
    ...
    describe('POST/ ', ()=>{
        ...
        it('should return 404 if genre is not found', async ()=>{
            genreId = new mongoose.Types.ObjectId();  // Assign a non-existent genreId
            movie.genre = genreId;
            const res = await execPost();
            expect(res.status).toBe(404);
        })

        it('should return 400 if movie is already created', async ()=>{
            let movie_db = await Movie.create({
                title: 'movie', 
                genre: genreId,
                numberInStock: 10,
                dailyRentalRate: 100,
